...
- (void)initView {
    ...
    self.ratioView.layer.borderColor = [UIColor lightGrayColor].CGColor;
...
- (void)initShowImage{
    ...
    if(self.cropFrame.size.height/self.cropFrame.size.width > self.originalImage.size.height/self.originalImage.size.width){
        [self.showImgView setFrame:CGRectMake(0,0 ,self.originalImage.size.width * self.cropFrame.size.height / self.originalImage.size.height,self.cropFrame.size.height)];
    }
    else {
        [self.showImgView setFrame:CGRectMake(0, 0, self.cropFrame.size.width, self.originalImage.size.height * self.cropFrame.size.width / self.originalImage.size.width)];
    }
    [self.showImgView setCenter:CGPointMake(self.view.bounds.size.width/2.0f, self.view.bounds.size.height/2.0f)];
...
- (void)initControlBtn {
    ...
    UIButton * rotaButton = [UIButton buttonWithType:UIButtonTypeCustom];

    [rotaButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    
    [rotaButton.titleLabel setText:@"旋转"];
    [rotaButton setBackgroundColor:[UIColor blackColor]];
    [rotaButton.layer setCornerRadius:25.0f];
    [cancelBtn .layer setCornerRadius:25.0f];
    [confirmBtn.layer setCornerRadius:25.0f];
    [cancelBtn setClipsToBounds:YES];
    [confirmBtn setClipsToBounds:YES];
    [rotaButton setClipsToBounds:YES];
    [rotaButton setTitle:@"旋转"forState:UIControlStateNormal];
    
    [rotaButton setFrame:CGRectMake(0, 0, 100, 50)];
    
    [rotaButton setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2.0, [UIScreen mainScreen].bounds.size.height -25)];
    [self.view addSubview:rotaButton];
    [rotaButton addTarget:self action:@selector(rotaView) forControlEvents:UIControlEventTouchUpInside];
}

- (void)rotaView{
    
    self.originalImage = [UIImage fixOrientation:[UIImage image:self.originalImage rotation:UIImageOrientationLeft]];
    
    [self.showImgView setImage:self.originalImage];
 
    [self initShowImage];
    [self handleBorderOverflow:self.showImgView.bounds];
}
...
- (CGRect)handleBorderOverflow:(CGRect)newFrame {
    ...
    if (newFrame.origin.x + newFrame.size.width < self.cropFrame.size.width + self.cropFrame.origin.x) newFrame.origin.x = self.cropFrame.size.width - newFrame.size.width +self.cropFrame.origin.x;
    // vertically
...
-(UIImage *)getSubImage{
   ...
   [[UIImage imageWithCGImage:subImageRef] drawInRect:CGRectMake(0, 0, size.width, size.height)];
