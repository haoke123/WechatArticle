...
+(instancetype) sharedNotiView{
    static AFDropdownNotification * sharedNoti=nil;
    
    if(sharedNoti==nil){
    
    
    
        sharedNoti = [[AFDropdownNotification alloc]init];
...
-(void)presentInView:(UIView *)view withGravityAnimation:(BOOL)animation {
    ...
    if (!_isBeingShown) {
        if(self.autoHide){
            ...
            [self performSelector:@selector(try2Hide) withObject:nil afterDelay:4];
        }
...
-(void) setSubtitleText:(NSString *)subtitleText{
   ...
   NSRange  range =  [subtitleText rangeOfString:@": "];
    ...
    if(range.location == NSNotFound){
     ...
     _subtitleText = subtitleText;
        ...
        NSString * tar= [subtitleText substringWithRange:NSMakeRange(range.location + range.length, subtitleText.length  - range.location -2)];
        NSString * name = [subtitleText substringWithRange:NSMakeRange(0, range.location + range.length)];
        
        NSDictionary * dict = [tar stringIsTypeofHouseSouse];
        if(dict){
            NSArray * array = @[name,dict[SOUSEType],dict[SOUSETitle1],dict[SOUSETitle2],dict[SOUSETitle3],dict[SOUSEContent1],dict[SOUSEContent2],dict[SOUSEContent3]];
            _subtitleText =  [array componentsJoinedByString:@" "];
