...
-(void)createTextFields
{
 ...
 //   user.keyboardType=UIKeyboardTypeWebSearch;
    user.clearButtonMode = UITextFieldViewModeWhileEditing;
   
    pwd=[self createTextFielfFrame:CGRectMake(60, 60, 271, 30) font:[UIFont systemFontOfSize:14]  placeholder:@"6-16位混合密码" ];
...
-(void)landClick
{
  ...
  MBProgressHUD * hud =   [MBProgressHUD showHUDAddedTo:self.view.window animated:YES];
    ...
    [JMSGUser loginWithUsername:user.text password:pwd.text completionHandler:^(id resultObject, NSError *error) {
        
        [hud hide:YES];
        
        if(error){
        
            [MBProgressHUD showError:@"登录失败" toView:self.view.window];
            
            [[NSNotificationCenter defaultCenter] postNotificationName:HKLoginMantualFail object:nil];
