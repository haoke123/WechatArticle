...
#define TopToView 20.0f
#define LeftToView 20.0f
...
- (instancetype)initWithDataArr:(NSArray *)dataArr origin:(CGPoint)origin width:(CGFloat)width rowHeight:(CGFloat)rowHeight
{
    ...
    if (self) {
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + LeftToView -10, origin.y + TopToView, width -10, rowHeight * dataArr.count) style:UITableViewStylePlain];
...
- (void)dismissWithCompletion:(void (^)(JHCustomMenu *object))completion
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.alpha = 0;
        weakSelf.tableView.frame = CGRectMake(weakSelf.frame.origin.x + weakSelf.bounds.size.width, weakSelf.origin.y + 0, 30, 0);
...
//    [colors[serie] setFill];
    ...
    CGContextMoveToPoint(context,
                        [UIScreen mainScreen].bounds.size.width - 120+ LeftToView * 2.5,66 + TopToView * 0.5);//设置起点
    
    CGContextAddLineToPoint(context,
                            [UIScreen mainScreen].bounds.size.width - 120 + LeftToView * 2,66 + TopToView);
    
    CGContextAddLineToPoint(context,
                           [UIScreen mainScreen].bounds.size.width - 120 + LeftToView * 3, 66 +TopToView);
